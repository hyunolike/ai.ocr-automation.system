...
### 0.2 `@Transactional` 이 적용되지 않는 자기 호출
...
| # | 항목 | 근거 |
|---|---|---|
| 1 | 0.1~0.4 결함 수정 | 이미 잘못 동작하고 있다 |
| 2 | 1.1 파이프라인 비동기화 | 0.1 의 근본 해결. 뒤 단계의 전제 |
| 3 | 1.2 소유자 도입 | 인가할 대상이 없으면 인증이 무의미 |
| 4 | 1.3 인증·인가 | `/internal` 이 공개 포트에 열려 있다 |
