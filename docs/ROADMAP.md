...
> **진행 상황** — Phase 0 의 결함 4건, 1.1(파이프라인 비동기화), 1.2(소유자 도입)를
> **완료**했다. 아래 해당 절에 ✅ 로 표시해 두었고, 무엇을 어떻게 했는지는 그대로 남긴다.
> 다음 차례는 **1.3 인증·인가**다.
>
> 1.2 는 설계와 한 가지가 달라졌다. 인증(1.3)이 아직 없으므로 소유자를
> `DocumentOwnerResolver` 포트로 끊고 임시로 `X-Owner-Id` 헤더를 읽게 했다.
> **인가할 대상(소유자)을 먼저 세우고, 증명(인증)은 1.3 에서 붙인다.**
...
| ~~2~~ | ~~1.1 파이프라인 비동기화~~ | ✅ 완료 |
| ~~3~~ | ~~1.2 소유자 도입~~ | ✅ 완료 |
| **4** | **1.3 인증·인가** | **← 다음 차례.** 소유자 격리는 섰지만 증명이 없다. `/internal` 도 공개 포트에 열려 있다 |
| 5 | 1.4 파일 검증 / 1.5 설정 암호화 | 함께 묶어 처리 가능 |
| 6 | 2.1 컨테이너 / 2.2 CI | 여기서부터 실제 배포 |
