...
현재 구현은 `X-Owner-Id` 헤더를 읽는 자리표시자다. 헤더는 누구나 보낼 수 있으므로
소유자를 **증명**하지는 못한다. 여기서 세운 것은 **인가할 대상**이다 —
소유자 개념이 없으면 인증을 붙여도 막을 것이 없다.

Phase 1.3 에서 API Key 를 해석하는 구현으로 교체하면, 바뀌는 것은 포트의 구현체
하나뿐이고 서비스와 도메인은 손대지 않는다. 기본 소유자를 두지 않은 것도 의도적이다 —
헤더 없이도 그럭저럭 돌아가면 그 상태가 운영까지 따라간다.
...
| 항목 | 영향 | 우선순위 |
|---|---|---|
| 인증 | 소유자 격리는 있으나 소유자를 증명하지 못한다. 헤더를 바꾸면 남의 문서가 보인다 | 높음 |
| `/internal` 보호 | 공개 포트에 열려 있어 외부에서 배치를 돌릴 수 있다 | 높음 |
