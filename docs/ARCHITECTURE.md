...
### 업로드는 헤더가 아니라 내용을 믿는다

`Content-Type` 은 클라이언트가 보낸 문자열이다. 그래서 파일 앞부분의 매직 바이트로
실제 형식을 확인한다. Apache Tika 를 쓰지 않은 이유: 허용 형식이 넷뿐이고 각 시그니처는
몇 바이트짜리 상수인데, Tika 는 수백 형식을 위해 의존성과 기동 시간을 가져온다.

PDF 는 한 걸음 더 간다. **암호화됐거나 지나치게 긴 문서**를 업로드 시점에 거른다.
통과시키면 OCR 단계에서야 드러나는데, 그때는 워커 하나가 붙잡혀 있고 재시도 한도만큼
낭비한 뒤다. 검증 비용을 앞으로 당기는 편이 싸다.

### 암호화는 키를 어디 두느냐가 전부다

설정 값 암호화(`{cipher}`) 기능은 켜져 있고 왕복이 검증돼 있다. 다만 저장소에는
**암호문을 커밋하지 않았다.** 개발 기본 키로 만든 암호문을 함께 두면 보안이 있는
것처럼 보이기만 하고 실제로는 없다. 키는 환경변수로만 주입한다.

접근 통제(기본 인증)가 1차 방어이고 암호화는 2차다. 순서를 뒤집으면 안 된다 —
암호화해도 `/encrypt` 가 열려 있으면 누구나 임의의 값을 그 키로 심을 수 있다.
...
| 운영 설정 암호문 적용 | 기능은 준비됐으나 값은 아직 환경변수 placeholder 다 | 중간 |
