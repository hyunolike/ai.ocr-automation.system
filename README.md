...
| ~~**1**~~ | ~~운영 투입 차단 해소~~ | ✅ 완료 — 비동기화, 소유자, 인증·인가, 파일 검증, 설정 암호화 |
| **2** | 배포 가능하게 | **← 다음.** 컨테이너 이미지, CI, 관측성, API 문서 |
| **3** | 인식 정확도 | 이미지 전처리, 신뢰도 수집, `NEEDS_REVIEW` 상태, PDF 페이지 처리 |
| **4** | 규모 | S3 어댑터, 보관 정책, 큐 전환 판단 |
| **5** | 구조화 추출 | 텍스트가 아니라 데이터를 준다 |

**Phase 1 이 끝났다.** 인증·인가, 소유자 격리, 파일 내용 검증이 모두 들어갔다.
다음은 **Phase 2 — 배포 가능하게**다. 컨테이너 이미지와 CI 가 없어 아직 서버에 올릴 수 없다.
